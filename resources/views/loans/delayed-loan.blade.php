...
@push('styles')
@endpush
@section('title', 'Manage Delayed Loan')

@section("page title", "Delayed Loan")
@section("breadcrumbs")
    <ol
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div class="rounded-lg border border-[--border-clr] bg-[--container-clr] p-6 shadow-sm">
            ...
            <div class="py-4 space-y-3">
                ...
                <div class="flex justify-between">
                    <span class="text-sm text-[--text-2-clr]">Tanggal Pinjam:</span>
                    <span class="text-sm text-[--text-clr]">{{ $loan->loan_date->format('d M Y H:i') }}</span>
                </div>

                <div class="flex justify-between">
                    <span class="text-sm text-[--text-2-clr]">Batas Kembali:</span>
                    <span class="text-sm text-[--text-clr]">{{ $loan->return_time->format('H:i') }}</span>
                </div>

                <div class="mt-3">
                    <p class="text-sm text-[--text-2-clr]">Deskripsi:</p>
                    <p class="text-sm text-[--text-clr]">{{ $loan->loan_description ?? '-' }}</p>
                </div>
            </div>

            <!-- Footer Card -->
            <div class="flex items-center justify-between border-t border-[--border-clr] pt-4 items-center">
                
                
                <div class="flex gap-2 items-center">
                    <span class="text-sm px-2 py-1 rounded self-start 
                    @if($loan->loan_status === 'delayed') bg-yellow-100 text-yellow-800
                    @elseif($loan->loan_status === 'approved') bg-green-100 text-green-800
                    @else bg-red-100 text-red-800 @endif">
                    {{ strtoupper($loan->loan_status) }}
                </span>
                    <form action="{{ route('loans.approve', $loan->loan_id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <button type="submit" class="px-3 py-1 text-sm rounded bg-green-500 text-white hover:bg-green-600">
                            Approve
                        </button>
                    </form>
                    
                    <form action="{{ route('loans.reject', $loan->loan_id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="px-3 py-1 text-sm rounded bg-red-500 text-white hover:bg-red-600">
                            Reject
                        </button>
                    </form>
                </div>
        ...
        <div class="col-span-full text-center py-12">
            <p class="text-[--text-2-clr]">Tidak ada peminjaman tertunda</p>
        </div>
        @endforelse
    </div>
</div>
